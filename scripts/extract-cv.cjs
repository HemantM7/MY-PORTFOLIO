...
async function main() {
  ...
  // pdf-parse may export a function or a default function depending on build
  const processor = typeof pdf === 'function' ? pdf : (pdf && pdf.default) ? pdf.default : null
  if (!processor) throw new Error('pdf-parse export not found')
  const data = await processor(dataBuffer)
